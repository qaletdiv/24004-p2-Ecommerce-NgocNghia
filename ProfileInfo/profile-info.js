    ...
    async function updateProfileImage() {
        ...
        try {
            ...
            if (data.authenticated && data.account) {
                // User logged in
                const userResponse = await fetch('http://localhost:3000/api/users/getProfile', {
                    method: 'GET',
                });
    
                if (userResponse.ok) {
                    const userData = await userResponse.json();
                    if (userData.user && userData.user.profile_user_image) {
                        profileLogo.src = userData.user.profile_user_image;
                        profileLogo.alt = userData.user.user_full_name || 'User profile';
                        return;
                    }
                }
                
                // Fallback: Use default image for logged in user
                profileLogo.src = "https://www.svgrepo.com/show/343494/profile-user-account.svg";
                profileLogo.alt = data.account.account_name || "User profile";
            } else {
                // Guest user
                profileLogo.src = "https://www.svgrepo.com/show/343494/profile-user-account.svg";
    ...
    async function goToProfile() {
        try {
            ...
            if (data.authenticated && data.user) {
                ...
                // Session không hợp lệ, redirect đến login
